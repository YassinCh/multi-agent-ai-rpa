(
  ...
  function highlight_element(element, index, parentIframe = null) {
   ...
   */
  function get_xpath_tree(element, stopAtBoundary = true) {
    const segments = [];
  ...
  function is_element_accepted(element) {
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE && !is_element_accepted(node)) {
      return null;
    }
    ...
    const nodeData = {
      ...
      attributes: {},
      xpath:
        node.nodeType === Node.ELEMENT_NODE ? get_xpath_tree(node, true) : null,
      children: [],
    };
        ...
        if (doHighlightElements) {
          if (focusHighlightIndex >= 0) {
            if (focusHighlightIndex === nodeData.highlightIndex) {
              highlight_element(node, nodeData.highlightIndex, parentIframe);
            }
          } else {
            highlight_element(node, nodeData.highlightIndex, parentIframe);
          }
